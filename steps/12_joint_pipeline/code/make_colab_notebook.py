"""Generate the Colab canonicalization notebook (canonicalize_gold.ipynb) that runs
Tomer's canonicalization (Gemma-4-31B, his exact prompt) over OUR gold segments.
Resumable, writes canonical_gold.json to Drive incrementally. CPU-only to generate."""
import json
...
def code(*s): return {"cell_type": "code", "metadata": {}, "outputs": [], "execution_count": None, "source": list(s)}

cells = [
md("# Canonicalize our gold segments (Gemma-4-31B, Tomer's prompt)\n",
   "\n",
...
code("# === config ===\n",
     ...
     "# Folder in YOUR My Drive (created for you). Upload canon_input.json into it.\n",
     "BASE     = Path('/content/drive/MyDrive/knesset_canon')\n",
     ...
     "print(CFG)"),
code("# === load Gemma-4-31B-it (4-bit nf4; fits ~18-20GB on an A100) ===\n",
     "import torch\n",
...
code("# === Hebrew-robust JSON extraction (Tomer's helpers) ===\n",
     "import re\n",
     "def _json_objects(s):\n",
     ...
     "def _salvage_json(s):\n",
     "    return re.sub(r'(?<=[\\u0590-\\u05FF])\"(?=[\\u0590-\\u05FF])', '\\\\\\\\\"', s)"),
code("# === canonicalization prompt (Tomer's exact CANON_SYSTEM + CANON_INSTRUCTIONS) ===\n",
     "import json\n",
     ...
     "    return c"),
code("# === resumable batch: canonicalize every segment, rewrite output after each ===\n",
...
md("## Get the result back to the cluster\n",
   "\n",
...
code("# Option B: scp the result directly to the cluster (edit USER/HOST/PATH; needs an SSH key in Colab)\n",
...
out = NB_DIR / "canonicalize_gold.ipynb"
